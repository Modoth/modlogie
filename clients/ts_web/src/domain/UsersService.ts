...
export default class UsersService extends IServicesLocator implements IUsersService {
    ...
    async all(filter?: string, skip?: number, take?: number): Promise<[number, User[]]> {
        var res = await ClientRun(() => this.locate(UsersServiceClient).getAll(new GetAllRequest().setFilter(filter || '').setTake(take || 0).setSkip(skip || 0), null));
    ...
    async add(name: string, email: string, password: string): Promise<User> {
        var res = await ClientRun(() => this.locate(UsersServiceClient).add(new AddRequest().setId(name).setEmail(email).setPassword(password || ''), null));
        var user = this.userFrom(res.getUser()!)
        return user;
    }

    async delete(name: string): Promise<void> {
        await ClientRun(() => this.locate(UsersServiceClient).delete(new StringId().setId(name), null));
    }

    async updateStatue(name: string, enable: boolean): Promise<void> {
        await ClientRun(() => this.locate(UsersServiceClient).updateStatus(new UpdateStatusRequest().setId(name).setUserStatus(enable ? UserDto.Status.ENABLED : UserDto.Status.DISABLED), null));
    }
    ...
    async updateType(name: string, authorised: boolean): Promise<void> {
        ...
        await ClientRun(() => this.locate(UsersServiceClient).updateType(request, null));
    }
    ...
    async updateAuthorisionExpired(name: string, expired: Date): Promise<void> {
        ...
        await ClientRun(() => this.locate(UsersServiceClient).updateAuthorisionExpired(request, null));
    }

    async updateComment(name: string, comment: string): Promise<void> {
        await ClientRun(() => this.locate(UsersServiceClient).updateComment(new UpdateCommentRequest().setId(name).setComment(comment), null));
    }
}