...
import { useArticleEnv, useServicesLocate } from '../../../view/common/Contexts'
...
export default function H5LiveViewer (props: AdditionalSectionViewerProps) {
  ...
  const reload = () => {
    buildIframeData(new Map(props.sections.map(s => [s.name!, s])), { locate, id: props.articleId, defaultFws: [], apiInfos: [], reload }).then(data => {
      if (data) {
        setData(data)
